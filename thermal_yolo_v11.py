...
while True:
    ...
    results = model.predict(grayImage, conf=0.2)  # Confidence threshold set to 0.5
    ...
    for result in results:
        for box in result.boxes:
            ...
            # Draw bounding box
            cv2.rectangle(grayImage, (x1, y1), (x2, y2), (0, 255, 0), 2)
            label = f"{model.names[cls]}: {conf:.2f}"
            cv2.putText(grayImage, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    # Show the frame
    cv2.imshow("YOLO Detection", grayImage)
